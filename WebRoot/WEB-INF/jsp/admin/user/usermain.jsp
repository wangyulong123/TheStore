...
<html>
	...
	<body>
	   ...
	   <div>
			<ul class="breadcrumb">
				<li><a href="#">首页</a></li>
				<li><a href="#">用户管理系统</a></li>
				<li><a href="#">管理用户</a></li>
			</ul>
		</div>
		
		<form id="categoryForm" action="UsernameServlet?action=getPageByQuery" method="post">
		...
		<div class="text-center categoryTableDiv">
					...
					%>
						...
						<tr>
							...
							<td>
								...
								<button class="btn btn-success btn-xs" onclick="getOneForUpdate(<%=user.getUserid() %>)">修改</button>
							</td>
							<td>
								<button class="btn btn-danger btn-xs" onclick="deleteCategoryById(<%=user.getUserid() %>)">删除</button>
			...
			<form id="categoryForm2" action="UsernameServlet?action=getPageByQuery" method="post">
			<input type="hidden" name="searchCondition" value="${requestScope.searchCondition}"/>
	<a id="first" class="btn btn-default btn-sm" href="UsernameServlet?action=getPageByQuery&searchCondition=${requestScope.searchCondition}&requestPage=1">首页</a>
    
    
    <a id="previous" class="btn btn-default btn-sm" href="UsernameServlet?action=getPageByQuery&searchCondition=${requestScope.searchCondition}&requestPage=<%=pageInfo.getPreviousPage()%>"> 上一页</a>
      <a id="next" class="btn btn-default btn-sm" href="UsernameServlet?action=getPageByQuery&searchCondition=${requestScope.searchCondition}&requestPage=<%=pageInfo.getNextPage()%>"> 下一页</a>
    
    
     <a id="last" class="btn btn-default btn-sm" href="UsernameServlet?action=getPageByQuery&searchCondition=${requestScope.searchCondition}&requestPage=<%=pageInfo.getTotalPageCount()%>">尾页</a>
    ...
    <span>页 </span>
		...
		<script type="text/javascript">
			...
			function deleteCategoryById(id){
				...
				$.confirm({
					...
					confirm: function() {
						...
						location.assign("UsernameServlet?action=delete&searchCondition=${requestScope.searchCondition}&requestPage=<%=pageInfo.getCurrentPage()%>&userid=" + id);
					},
					cancel: function() {
			...
			function getOneForUpdate(id){
				...
				location.assign("UsernameServlet?action=getOneForUpdate&searchCondition=${requestScope.searchCondition}&requestPage=<%=pageInfo.getCurrentPage()%>&userid=" + id);
			}
