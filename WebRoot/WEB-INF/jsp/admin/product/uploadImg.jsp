	...
	<body>
		...
		<div>
			<ul class="breadcrumb">
				...
				<li><a href="javascript:void(0)">上传图片</a></li>
			</ul>
		</div>
		
		<form id="categoryForm" action="ProductServlet?action=getPageByQuery&target=uploadImg" method="post">
		...
		<div class="text-center categoryTableDiv">
					...
					<%
						...
						<tr>
							...
							<td>
								...
								<button class="btn btn-success btn-xs" onclick="getOneForUpload(<%=product.getPid() %>)">上传</button>
							</td>
						</tr>
			...
			<form id="categoryForm2" action="ProductServlet?action=getAllByPage&target=uploadImg" method="post">
   ...
   <span>共 <%=pageInfo.getTotalRecordCount()%> 条记录， </span>
    ...
    <span>页 </span>
		...
		<ul class="pagination" style="position:relative;right:30px;float:right;">
			<li ><a href="ProductServlet?action=getAllByPage&target=uploadImg&recordsPerPage=<%=pageInfo.getPerPageRecordCount() %>&searchCondition=${requestScope.searchCondition}&requestPage=1">首页</a></li>
			<li ><a href="ProductServlet?action=getAllByPage&target=uploadImg&recordsPerPage=<%=pageInfo.getPerPageRecordCount() %>&searchCondition=${requestScope.searchCondition}&requestPage=<%=pageInfo.getPreviousPage()%>">&laquo;</a></li>
			...
			<li><a href="ProductServlet?action=getAllByPage&target=uploadImg&recordsPerPage=<%=pageInfo.getPerPageRecordCount() %>&searchCondition=${requestScope.searchCondition}&requestPage=<%=i %>"><%=i %></a></li>
			<% 
				}
			 %>
			
			<li><a href="ProductServlet?action=getAllByPage&target=uploadImg&recordsPerPage=<%=pageInfo.getPerPageRecordCount() %>&searchCondition=${requestScope.searchCondition}&requestPage=<%=pageInfo.getNextPage()%>">&raquo;</a></li>
			<li><a href="ProductServlet?action=getAllByPage&target=uploadImg&recordsPerPage=<%=pageInfo.getPerPageRecordCount() %>&searchCondition=${requestScope.searchCondition}&requestPage=<%=pageInfo.getTotalPageCount()%>">尾页</a></li>
		</ul>
		...
		<script type="text/javascript">
	 	$(function(){
			 ...
			 %>
				 location.assign("ProductServlet?action=getAllByPage&target=uploadProductImg&recordsPerPage=<%=pageInfo.getPerPageRecordCount() %>&searchCondition=${requestScope.searchCondition}&requestPage=1");
			  <% 	
		      	}
		      %>
	 	});
		...
		<script type="text/javascript">
			...
			function deleteCategoryById(id){
				...
				$.confirm({
					...
					confirm: function() {
						...
						location.assign("ProductServlet?action=delete&recordsPerPage=<%=pageInfo.getPerPageRecordCount() %>&searchCondition=${requestScope.searchCondition}&requestPage=<%=pageInfo.getCurrentPage()%>&pid=" + id);
					},
					cancel: function() {
			...
			function getOneForUpload(id){
				
				location.assign("ProductServlet?action=getOneForUpload&target=uploadProductImage&recordsPerPage=<%=pageInfo.getPerPageRecordCount() %>&searchCondition=${requestScope.searchCondition}&requestPage=<%=pageInfo.getCurrentPage()%>&pid=" + id);
			}
		</script>
		<%
